// Package delve provides a client for headless Delve sessions and
// high-level wrappers for all debugging commands.
package delve
...
// DefaultAddrFile is the path where the Delve listen address is written.
const DefaultAddrFile = ".dlv/addr"
...
func newRPCLogger() (*rpcLogger, error) {
	...
	if val == "1" || strings.EqualFold(val, "true") {
		path = filepath.Join(".dlv", "rpc.log")
	}
...
func getAddr() (string, error) {
	...
	b, err := os.ReadFile(DefaultAddrFile)
	if err != nil {
		return "", fmt.Errorf("no DLV_ADDR and %s not found: %w (start headless dlv and set DLV_ADDR or write address to %s)", DefaultAddrFile, err, DefaultAddrFile)
	}
...
// withSession connects to the running Delve instance, fetches state, and calls fn.
// If the debuggee has already exited, it prints the exit message and returns nil.
func withSession(fn func(*loggingClient, *api.DebuggerState) error) error {
