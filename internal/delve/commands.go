// Delve session commands exposed as package-level functions.
package delve
...
// Exported command functions — each opens its own session via withSession.

func State() error {
	return withSession(func(c *loggingClient, s *api.DebuggerState) error {
		return printState(s)
	})
}
...
func Break(args []string) error {
	return withSession(func(c *loggingClient, s *api.DebuggerState) error {
		return cmdBreak(c, s, args)
	})
}

func Breakpoints() error {
	return withSession(func(c *loggingClient, _ *api.DebuggerState) error {
		return cmdBreakpoints(c)
	})
}

func Clear(args []string) error {
	return withSession(func(c *loggingClient, _ *api.DebuggerState) error {
		return cmdClear(c, args)
	})
}

func Continue() error {
	return withSession(func(c *loggingClient, _ *api.DebuggerState) error {
		return cmdContinue(c)
	})
}

func Next() error {
	return withSession(func(c *loggingClient, _ *api.DebuggerState) error {
		return cmdStep(c, api.Next)
	})
}

func Step() error {
	return withSession(func(c *loggingClient, _ *api.DebuggerState) error {
		return cmdStep(c, api.Step)
	})
}

func StepOut() error {
	return withSession(func(c *loggingClient, _ *api.DebuggerState) error {
		return cmdStep(c, api.StepOut)
	})
}

func Print(args []string) error {
	return withSession(func(c *loggingClient, s *api.DebuggerState) error {
		return cmdPrint(c, s, args)
	})
}

func Locals() error {
	return withSession(func(c *loggingClient, s *api.DebuggerState) error {
		return cmdLocals(c, s)
	})
}
